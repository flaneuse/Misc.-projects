                        ...
                        ifelse(no_lists == 3, 1, 
                               ifelse(frag_usaid == 'Fragile' & frag_fsi == 'Fragile', 2,
                                      ifelse(frag_usaid == 'Fragile' & frag_wb == 'Fragile', 3,
                                             ifelse(frag_usaid == 'Fragile', 4,
                                                    ifelse(frag_fsi == 'Fragile', 5,
                                                           ifelse(frag_wb == 'Fragile', 6, 7)))))))
...
df_tidy =  df_tidy %>% 
  select(country, region, contains('frag'), fill_color, no_lists, order) %>% 
...
df_tidy$list_name = factor(df_tidy$list_name, 
                           levels = c('frag_usaid', 'frag_fsi', 'frag_wb'),
                           labels = c('USAID', 'FSI', 'World Bank'))
...
dot_matrix = function(df,
  ...
  ggplot(df, aes_string(x = x_var, 
    ...
    scale_fill_manual(values = c('0' = no_colour, 'Fragile' = yes_colour)) +
    theme_xylab()
}
